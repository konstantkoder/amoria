...
Updated: 2026-06-07 for `PROFILE-MEDIA-STABILITY-01`.
...
| --- | --- | --- | --- |
| A | Avatar upload crop | NOT TESTED | Login account A, open Profile, choose a normal camera/photo-library JPEG/PNG/WebP avatar, verify full-screen dark crop screen opens slightly zoomed with source image under a fixed square frame, dimmed outside area, visible source context around the square, 3x3 grid only inside the square, no `+` / `-` crop controls, no Reset crop control, and bottom actions outside the square. Drag face/object under the frame with one finger on both axes, pinch zoom with two fingers around the touch midpoint, tap `Готово`, verify unsaved preview appears with `Предпросмотр — ещё не сохранено`, tap Upload avatar, verify new avatar appears from backend, restart app, verify avatar persists, open public profile from account B, verify peer sees avatar. |
| B | Profile photo upload crop | NOT TESTED | Open PhotoManager, choose a normal camera/photo-library JPEG/PNG/WebP profile photo, verify full-screen crop screen opens slightly zoomed with dimmed outside area, visible source context around the fixed square, and grid only inside the square. Drag/pinch naturally, verify the image always fills the crop square with no black/empty final crop area, tap `Готово`, verify preview appears and upload has not happened yet, tap `Загрузить`, verify backend-mediated `POST /media/profile-photo` result appears cropped in public gallery and matches the preview, refresh owner gallery, restart app, verify it persists, login peer account, verify peer can see public photo, verify URL is relative `/media/public/:mediaId` or current backend origin plus that path. |
| B1 | Peer avatar/profile media | NOT TESTED | Account A uploads avatar and public profile photo; account B opens A from Together DM/profile context and sees current backend public media URLs derived from media ids or a clear moderation-waiting explanation if policy changes to approved-only. If an image fails, Admin Client Errors should show `screen=UserProfileScreen`, `action=loadAvatar` or `loadPublicPhoto`, safe `mediaId` when known, `urlKind`, `httpStatus`, `contentType`, `hasAvatarUrl`, `photoCount`, and safe `visibility`, never a raw URL. |
| B2 | Crop cancel/change | NOT TESTED | Select avatar/photo, reach crop screen, tap `Отменить` and verify no upload/avatar/gallery change. Repeat and tap `Выбрать другое`; verify the selected image is replaced before upload and only the confirmed replacement can upload. |
...
| Profile media loading stability | PASS | Profile, UserProfile, Nearby cards, PhotoManager, and shared avatar rendering use stable loading/fallback frames. Public photo and avatar failures report only safe `loadAvatar`, `loadPublicPhoto`, or `loadOwnerPhoto` diagnostics. |
...
| Avatar/profile crop action visibility | PASS | Mobile no longer depends on the native cropper action bar; avatar and profile photo upload wait for visible full-screen in-app crop, preview, and explicit upload actions. |
| Crop stage release UX | PASS | Crop screen shows a fixed square frame, dimmed outside area, 3x3 grid only inside the square, starts at `minCoverScale * 1.08`, supports one-finger image drag, two-finger focal-point pinch zoom, and clamps pan/zoom so the square stays filled. Done/Cancel/Choose another stay outside the square; Reset and `+` / `-` crop controls are not rendered in current crop code. Test with a normal camera/photo-library image; if old `+` / `-` controls appear only when cropping an old screenshot, they are pixels inside the chosen image, not crop UI. |
