...
Updated: 2026-05-29 for `BUGFIX-CROP-UX-RELEASE-SMOKE-07`.
...
| --- | --- | --- | --- |
| A | Avatar upload crop | NOT TESTED | Login account A, open Profile, choose JPEG/PNG/WebP avatar, verify full-screen dark crop screen opens with source image under a fixed square frame, dimmed outside area, 3x3 grid only inside the square, no dominant `+` / `-` zoom buttons, and bottom actions outside the square. Drag face/object under the frame with one finger, pinch zoom with two fingers around the touch midpoint, tap `Готово`, verify unsaved preview appears with `Предпросмотр — ещё не сохранено`, tap Upload avatar, verify new avatar appears from backend, restart app, verify avatar persists, open public profile from account B, verify peer sees avatar. |
| B | Profile photo upload crop | NOT TESTED | Open PhotoManager, choose JPEG/PNG/WebP profile photo, verify full-screen crop screen opens with dimmed outside area and grid, drag/pinch, verify the image always fills the crop square with no black/empty final crop area, tap `Готово`, verify preview appears and upload has not happened yet, tap `Загрузить`, verify backend-mediated `POST /media/profile-photo` result appears cropped in public gallery, refresh owner gallery, restart app, verify it persists, login peer account, verify peer can see public photo, verify URL is relative `/media/public/:mediaId` or current backend origin plus that path. |
...
| Crop stage release UX | PASS | Crop screen shows a fixed square frame, dimmed outside area, 3x3 grid only inside the square, one-finger image drag, two-finger focal-point pinch zoom, clamped pan/zoom that keeps the square filled, Done/Cancel/Choose another outside the square, and Reset as a small secondary action. |
