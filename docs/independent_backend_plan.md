...
Amoria is moving away from Firebase/Google so the app can control its own data model, hosting location, backups, media storage, auth lifecycle, and future realtime behavior. Firebase remains in the mobile app during the migration; this plan is about adding the independent backend without breaking the current app.
...
3. Custom auth cutover: replace Firebase auth only after backend auth and token storage are ready.
4. Chats: add persistence and realtime delivery outside Firebase.
...
8. Remove Firebase: delete Firebase dependencies only after every flow is fully migrated and verified.
...
Backups should be encrypted when stored outside the trusted server, tested with restore drills, and retained with a clear schedule before Firebase is removed.
