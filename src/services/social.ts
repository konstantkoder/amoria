...
function sortedId(a:string,b:string){ return [a,b].sort().join('_'); }
...
export async function likeUser(fromUid: string, toUid: string) {
  if (!isFirebaseConfigured()) {
    ...
    if (!arr.includes(toUid)) { arr.push(toUid); await AsyncStorage.setItem(key, JSON.stringify(arr)); }
    return await checkMatchLocal(fromUid, toUid);
  }
  await addDoc(collection(db, 'likes'), { from: fromUid, to: toUid, createdAt: Date.now() });
  // Check reciprocal like
  const q = query(collection(db, 'likes'), where('from','==',toUid), where('to','==',fromUid));
  const snap = await getDocs(q);
  if (!snap.empty) {
    // create match
    const id = sortedId(fromUid, toUid);
    await setDoc(doc(db, 'matches', id), { id, users: [fromUid, toUid], createdAt: Date.now() });
    return true;
  }
  return false;
}
