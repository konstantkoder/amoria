...
export default function RandomChatScreen() {
    ...
    (async () => {
      ...
      if (isFirebaseConfigured()) {
        ...
        if (!res.empty) {
          ...
          await updateDoc(doc(db, "randomRooms", room.id), {
            users: [room.data().users[0], id],
          });
          setRoomId(room.id);
        } else {
          const nd = await addDoc(collection(db, "randomRooms"), {
            users: ["__empty__", id],
            createdAt: Date.now(),
          });
          setRoomId(nd.id);
        ...
        const mq = query(
          collection(db, "randomRooms", roomId || "tmp", "messages"),
        );
  ...
  async function send() {
    if (!text.trim()) return;
    if (isFirebaseConfigured()) {
      await addDoc(collection(db, "randomRooms", roomId, "messages"), {
        from: uid,
        text,
        createdAt: Date.now(),
      });
