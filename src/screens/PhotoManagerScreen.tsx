...
import { ensureAuth, getUserProfile, updateUserPhotos } from '@/services/firebase';
...
export default function PhotoManagerScreen(){
  ...
  async function addPhoto(){
    ...
    if (status !== 'granted') {
      Alert.alert('Нет доступа', 'Разрешите доступ к фото, чтобы загрузить изображение');
      return;
    }
  ...
  return (
    <View style={{ flex:1, padding:16, backgroundColor: theme.colors.bg }}>
      <Text style={{ fontSize:22, fontWeight:'700', marginBottom:12 }}>Мои фото</Text>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <View style={{ margin:8, width:'46%', aspectRatio:1, borderRadius:12, overflow:'hidden', backgroundColor:'#eee' }}>
            <Image source={{ uri: item }} style={{ width:'100%', height:'100%' }} />
            <Button title="Удалить" onPress={() => removePhoto(index)} />
          </View>
        )}
      />
      {photos.length === 0 && <Text style={{ marginTop:12, opacity:0.7 }}>Пока нет фото. Добавь первое!</Text>}
    </View>
  );
}
